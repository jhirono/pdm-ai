// src/utils/parsers/models/claude-parser.js
...
class ClaudeParser extends BaseParser {
  ...
  /**
   * Call the Claude API specifically for scenario extraction
   * @param {string} content - The content to parse
   * @param {Object} fileInfo - Information about the file
  ...
  async callModelAPIForScenarios(content, fileInfo, specializedPrompt) {
    try {
      console.log("Calling Claude API for scenario extraction...");
      ...
      console.error(`Claude API call for scenario extraction failed: ${apiError.message}`);
      return { scenarios: [] };
    }
  }
...
IDは生成しなくても構いません。システム側で自動的に生成します。
...
You don't need to generate IDs - our system will automatically assign them.
...
IMPORTANT: Respond with ONLY the JSON object. Do not include any text outside the JSON.`;
  ...
  extractJSON(response) {
    ...
    try {
      ...
      // If only scenarios are found, construct a valid object with just scenarios
      if (scenariosMatch) {
        const scenariosJson = `{"scenarios": ${scenariosMatch[1]}}`;
        parsedResponse = JSON.parse(scenariosJson);
        console.log("Successfully extracted scenarios array only");
        return parsedResponse;
      }
    } catch (e) {
