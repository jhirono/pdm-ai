...
function generateJtbdCentricView(data, options, stats) {
  ...
  if (hasValidScenarios) {
    ...
    if (relevantScenarios.length > remainingNodeSlots) {
      // Sort by priority if available
      relevantScenarios.sort((a, b) => (b.priority || 0) - (a.priority || 0));
      relevantScenarios = relevantScenarios.slice(0, remainingNodeSlots);
    }
    ...
    // Add connections from concrete JTBDs to scenarios only
    diagram += '  %% JTBD to Scenario relationships\n';
    
    // Process only concrete JTBDs (remove abstract JTBD connections to scenarios)
    concreteJtbds.forEach(jtbd => {
      if (jtbd.relatedScenarios && Array.isArray(jtbd.relatedScenarios)) {
        jtbd.relatedScenarios.forEach(scenarioId => {
          if (scenarioMap[scenarioId]) {
            const jtbdId = makeIdSafe(jtbd.id);
            const scenId = makeIdSafe(scenarioId);
            diagram += `  ${jtbdId} --> ${scenId}\n`;
            stats.edgeCount++;
          }
        });
      }
